...
def _get_normal_dist_samples(mean, std, num_samples, r_state):
    return norm.rvs(size=num_samples, loc=mean, scale=std, random_state=r_state)
...
def get_resulting_density(i, demand_df, solar_df, onshore_df, offshore_df, ror_df,supply_df):
    ...
    if exp <= 0.3:
        ...
        pdf_array_demand_solar, cdf_array_demand_solar = get_densitys(
            ...
            lambda d: beta_solar_cdf(-d / scaleparam_solar),
            [theta_demand_solar],
            plt_title="Demand - Solar : " + str(i) ,
    ...
    pdf_array_demand_solar_on, cdf_array_demand_solar_on = get_densitys(
        ...
        lambda d: gamma.cdf(-d, a, scale=scale_onshore),
    ...
    pdf_array_demand_solar_on_off, cdf_array_demand_solar_on_off = get_densitys(
        ...
        lambda d: beta_offshore_cdf(-d / scaleparam),
    ...
    pdf_array_demand_solar_on_off_ror, cdf_array_demand_solar_on_off = get_densitys(
        ...
        lambda d: beta_ror_cdf(-d / scaleparam),
    ...
    with open(model_path + "/results_combined/" + str(i) + '.pickle', 'wb') as handle:
...
class CombinedDensity:
    ...
    def get_density_value(self, z):
        ...
        point_x = np.array([z - self.max_x - 0.1, z - self.max_x - 0.05, z - self.max_x, z - self.max_x + 0.05, z - self.max_x + 0.1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-folder', type=pathlib.Path, help="Path of the results")
    ...
    # print(mismatch_df["demand"][i], mismatch_df["solar"][i])
    ...
    offshore_samples = mismatch_df['offwind-ac'][0:35040] + mismatch_df['offwind-dc'][0:35040] + mismatch_df['curtailment-offwind-ac'][0:35040] + mismatch_df['curtailment-offwind-dc'][0:35040]
    ...
    demand_minus_solar = mismatch_df["demand"][0:35040] - mismatch_df["solar"][0:35040] + mismatch_df["curtailment-solar"][0:35040]
    ...
    with Pool() as p:
        ...
        res = p.starmap(get_resulting_density,
                        [(i, demand_df, solar_df, onshore_df, offshore_df, ror_df,supply_df) for i in range(args.lb, args.hb)])
