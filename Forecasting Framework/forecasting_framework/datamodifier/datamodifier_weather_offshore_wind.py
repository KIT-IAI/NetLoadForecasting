...
class DatamodifierWeatherOffshoreWind_2010_2020(Datamodifier):
    def apply(self, df_old, params_dict=None):
        df_old = pd.DataFrame(df_old)
        dfs = [
            pd.read_csv("datamodifier/Germany_Reanalysis_Data_Wind_Aggregated_Offshore_" + str(x) + ".csv", index_col=0)
