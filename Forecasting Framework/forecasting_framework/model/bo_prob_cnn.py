...
from forecasting_framework.model.model_prob_cnn import ProbCNN
...
#
# Example code CV with Keras Tuner
#
...
class BoProbCNN(Model):
    ...
    def execute_train(self):
        ...
        for i in range(x.shape[0] - 48):
            ...
            #
            # Scalings to fit distributions
            #

            if self.scaler_forecast_varaibles.inverse_transform(y[48 + i]) == 0 and self.modelparams_dict[
                "dist"] == "gamma":
                y_split_res.append(self.scaler_forecast_varaibles.inverse_transform(y[48 + i]) + 0.01)

            elif self.scaler_forecast_varaibles.inverse_transform(y[48 + i]) == 0 and self.modelparams_dict[
                "dist"] == "beta":
                y_split_res.append(
                    (self.scaler_forecast_varaibles.inverse_transform(y[48 + i]) + 0.01) / self.modelparams_dict[
                        "scaleparam"])

            elif self.modelparams_dict["dist"] == "beta_with_min_max":
                y_split_res.append(((self.scaler_forecast_varaibles.inverse_transform(y[48 + i])) -
                                    self.modelparams_dict["scaleparam_min"]) / (
                                       (self.modelparams_dict["scaleparam_max"] - self.modelparams_dict[
                                           "scaleparam_min"])))

            elif self.modelparams_dict["dist"] == "beta":
                y_split_res.append(
                    (self.scaler_forecast_varaibles.inverse_transform(y[48 + i])) / self.modelparams_dict["scaleparam"])
            else:
                y_split_res.append(self.scaler_forecast_varaibles.inverse_transform(y[48 + i]))
        ...
        cnn = ProbCNN(feature_size=features_train_val[0].shape[2], lag_size=features_train_val[1].shape[2],
                      dist=self.modelparams_dict["dist"])
        ...
        tuner = CVTuner(
            hypermodel=cnn.get_prob_cnn_model,
        ...
        for train_indices, test_indices in cv.split(features_train_val[0]):
            ...
            tmp_model = cnn.get_prob_cnn_model(best_hp)
    ...
    def execute_forecast(self):
        ...
        if self.modelparams_dict["dist"] == "normal":
            ...
            # results_of_all_models = np.array([x.predict(self.to_predict_features) for x in self.localmodel])
            all_models_concentration = pd.DataFrame(results_concentration.squeeze().T)
            all_models_rate = pd.DataFrame(results_rate.squeeze().T)
            all_models = pd.concat([all_models_concentration, all_models_rate], axis=1)
            ...
            variable = str(self.forecast_variables[0])
            hat_variable = str(self.forecast_variables[0]) + "_hat"
            testset_to_save[hat_variable] = concentration
            hat_variable = str(self.forecast_variables[0]) + "_hat"
